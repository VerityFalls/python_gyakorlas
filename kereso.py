...
def keres(katalogus, szerzo=None, cim=None, kiadas=None):
    for elem in katalogus:
        ...
        if "katalogus" in elem:
            ...
            print(str(elem["forras"]) + " katalógusából visszatért eredmény: " + str(eredmeny))
        elif "cim" in elem:
            if cim is None or cim in elem["cim"]:
                if szerzo is None or szerzo in elem["szerzo"]:
                    if kiadas is None or (isinstance(kiadas, int) and kiadas == elem["kiadas"]) or (isinstance(kiadas, tuple) and kiadas[0] <= elem["kiadas"] <= kiadas[1]):
                        print("Talált könyv: " + str(elem))
                        return elem
